...
def test_valid_expressions(validator: ValidatorInterface, tokens):
...
def test_empty_parentheses_throws_error(validator: ValidatorInterface):
...
def test_unbalanced_extra_closing_paren_throws_error(validator: ValidatorInterface):
...
def test_unbalanced_extra_opening_paren_throws_error(validator: ValidatorInterface):
...
def test_insufficient_operands_for_unary_op(validator: ValidatorInterface, tokens):
    """Не хватает операндов для унарных операторов"""
    with pytest.raises(InsufficientOperandsError):
        validator.validate(tokens)


def test_sub_expression_not_resolving_to_one_value(validator: ValidatorInterface):
